...
namespace EzSeries.Champions.Nidalee
{
    ...
    public abstract class Menu
    {
        ...
        public static void Initialize(Tab parentTab)
        {
            ...
            var allies = UnitManager.AllyChampions.OrderBy(a => a.UnitComponentInfo.SkinName);
            foreach (var a in allies)
            {
                _main.AddItem(new Switch("Auto heal " + a.UnitComponentInfo.SkinName, true));
                _main.AddItem(new Counter(a.UnitComponentInfo.SkinName + " heal pct", 75, 1, 100));
            }
            _config.AddGroup(_main);

            _clear = new Group("Clearing");
        ...
        {
            return _main.GetItem<Switch>("Auto heal " + unit.UnitComponentInfo.SkinName).IsOn;
        }
        
        public static int AutoHealAllyPct(AIBaseClient unit)
        {
            return _main.GetItem<Counter>(unit.UnitComponentInfo.SkinName + " heal pct").Value;
        }
